...
const EditSubscription = ({ subsData, setShowmenu, showSubmenu }) => {
    const [createSubscription, setCreateSubscription] = useState({
        ...
        discount_price: subsData?.discount_price,
        food_category: 'Veg',
        meal_type: 'breakfast',
        imgUrl: subsData?.img,
        sunday: subsData?.sunday?._id,
        monday: subsData?.monday?._id,
        tuesday: subsData?.tuesday?._id,
        wednesday: subsData?.wednesday?._id,
        thursday: subsData?.thursday?._id,
        friday: subsData?.friday?._id,
        saturday: subsData?.saturday?._id,
    })
    ...
    const handleFormSubmit = () => {
        ...
        editSubscriptionData(formData, subsData?.slug).then((res) => {
            if (res?.status === 200) {
        ...
        <div className={styles.category_container}>
            ...
            <div className={styles.form_row}>
                ...
                <div className={styles.input_container}>
                    ...
                    <select name='meal' className={styles.select_input} value={createSubscription?.meal_type} onChange={(e) => setCreateSubscription({ ...createSubscription, meal_type: e.target.value })}>
                        {
                            mealType?.map((ele) => {
                                return (
                                    <option value={ele?.value}>{ele?.name}</option>
